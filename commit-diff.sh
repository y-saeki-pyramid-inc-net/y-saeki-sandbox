...
(git rev-list --reverse --topo-order --no-merges $from^2..$develop_branch --ancestry-path | cat -n; git rev-list --reverse --topo-order --no-merges $from^2..$stage_branch --first-parent | cat -n) | sort -k2 | uniq -f1 -u | sort -n | cut -f2 | while read rev 
do
